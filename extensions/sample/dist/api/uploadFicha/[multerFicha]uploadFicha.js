...
export default async function uploadFicha(request, response) {
    ...
    try {
        ...
        const { rows: arows } = await pool.query(`SELECT attribute_id FROM attribute WHERE attribute_code = $1 LIMIT 1`, [FICHA_ATTR_CODE]);
        if (!arows.length) {
        ...
        const { rows: prows } = await pool.query(`SELECT sku FROM product WHERE product_id = $1`, [productId]);
        if (!prows.length) {
            return response
                .status(404)
        ...
        const { rows: exist } = await pool.query(`SELECT product_attribute_value_index_id
         FROM product_attribute_value_index
        WHERE product_id = $1 AND attribute_id = $2`, [productId, attributeId]);
        if (exist.length) {
            await pool.query(`UPDATE product_attribute_value_index
            SET option_text = $1, option_id = NULL
          WHERE product_id = $2 AND attribute_id = $3`, [url, productId, attributeId]);
        }
        else {
            await pool.query(`INSERT INTO product_attribute_value_index
           (product_id, attribute_id, option_id, option_text)
         VALUES ($1, $2, NULL, $3)`, [productId, attributeId, url]);
        }
...
//# sourceMappingURL=%5BmulterFicha%5DuploadFicha.js.map