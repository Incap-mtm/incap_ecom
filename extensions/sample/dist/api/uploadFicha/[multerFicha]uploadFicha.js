...
const sanitize = (s)=>(s || '').normalize('NFD').replace(/[̀-ͯ]/g, '').replace(/[^a-zA-Z0-9._-]+/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '').toLowerCase();
/**
 ...
 */ export default async function uploadFicha(request, response) {
    ...
    try {
        ...
        if (file.size > MAX_BYTES) {
            return response.status(400).json({
                success: false,
                error: 'El PDF supera el límite de 1 MB.'
            });
        }
        ...
        const { rows: arows } = await pool.query(`SELECT attribute_id FROM attribute WHERE attribute_code = $1 LIMIT 1`, [
            FICHA_ATTR_CODE
        ]);
        if (!arows.length) {
        ...
        const { rows: prows } = await pool.query(`SELECT sku FROM product WHERE product_id = $1`, [
            productId
        ]);
        if (!prows.length) {
