...
export default async function advisor(request, response) {
  try {
    ...
    const catalog = result.rows.map(p => {
      ...
      if (p.usos)           lines.push(`Usos: ${stripHtml(p.usos).substring(0, 200)}`);
      if (p.caracteristicas) lines.push(`Características: ${stripHtml(p.caracteristicas).substring(0, 200)}`);
      if (p.modo_empleo)    lines.push(`Aplicación: ${stripHtml(p.modo_empleo).substring(0, 150)}`);
      return lines.join('\n');
    }).join('\n---\n');
    ...
    if (skus.length > 0) {
      const prodRes = await db.query(
        `SELECT p.sku, pd.name, pd.url_key, cd.url_key AS cat_url
         FROM product p
         JOIN product_description pd ON pd.product_description_product_id = p.product_id
         LEFT JOIN product_category pc ON pc.product_id = p.product_id
         LEFT JOIN category cat ON cat.category_id = pc.category_id
