...
export default async function syncVariants(request, response) {
    ...
    try {
        const { rows: attrRows } = await client.query(`SELECT attribute_id FROM attribute WHERE attribute_code = 'size' LIMIT 1`);
        if (!attrRows.length) {
            client.release();
        ...
        const { rows: grpRows } = await client.query(`SELECT attribute_group_id FROM attribute_group WHERE group_name = 'Default' LIMIT 1`);
    ...
    `);
        ...
        for (const p of products) {
            ...
            if (normSize !== rawSize) {
                ...
                await client.query('UPDATE product_description SET name = $1 WHERE product_description_id = $2', [newName, p.product_description_id]);
                p.name = newName;
                renamed++;
            }
        }
        ...
        const { rows: existingOpts } = await client.query('SELECT attribute_option_id, option_text FROM attribute_option WHERE attribute_id = $1', [SIZE_ATTRIBUTE_ID]);
        ...
        for (const size of sizesNeeded) {
            if (!optionMap.has(size)) {
                const { rows: [opt] } = await client.query(`INSERT INTO attribute_option (uuid, attribute_id, attribute_code, option_text)
        ...
        for (const [, ps] of toMigrate) {
            ...
            if (variantGroupId === null) {
                const { rows: [vg] } = await client.query(`INSERT INTO variant_group (uuid, attribute_group_id, attribute_one, visibility)
           VALUES ($1, $2, $3, false) RETURNING variant_group_id`, [crypto.randomUUID(), ATTRIBUTE_GROUP_ID, SIZE_ATTRIBUTE_ID]);
            ...
            for (const p of newProducts) {
                ...
                await client.query('UPDATE product SET variant_group_id = $1 WHERE product_id = $2', [variantGroupId, p.product_id]);
                await client.query('DELETE FROM product_attribute_value_index WHERE product_id = $1 AND attribute_id = $2', [p.product_id, SIZE_ATTRIBUTE_ID]);
                await client.query(`INSERT INTO product_attribute_value_index (product_id, attribute_id, option_id, option_text)
