    ...
    try {
        ...
        try {
            ...
            if (Array.isArray(blogData.posts)) {
                blogEntries = blogData.posts.map((post) => urlEntry({ loc: `/blog/${post.slug}`, priority: '0.7', changefreq: 'monthly' }));
            }
        }
        catch (_a) { }
        // URLs canónicas limpias: categorías (4) + productos activos (322).
        const { rows } = await pool.query(`SELECT DISTINCT ur.request_path, ur.entity_type
         FROM url_rewrite ur
         LEFT JOIN product p
        ...
        const dbEntries = rows.map((row) => urlEntry({
