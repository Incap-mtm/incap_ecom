...
function buildQuery(keyword) {
    ...
    query {
      ...
      products(filters: [
        { key: "keyword", operation: eq, value: "${keyword.replace(/"/g, '')}" }
    ...
    return (React.createElement("div", { className: "min-h-screen bg-white font-sora", style: { paddingTop: '124px' } },
        React.createElement("section", { style: { background: 'linear-gradient(160deg, #2A4899 0%, #1e3576 100%)', padding: '40px 0 48px' } },
            React.createElement("div", { style: { maxWidth: '1536px', margin: '0 auto', padding: '0 2rem' } },
                ...
                React.createElement("h1", { style: { fontSize: 'clamp(2rem, 5vw, 3.5rem)', fontWeight: 900, color: '#fff', fontFamily: "'Sora', sans-serif", lineHeight: 1.1, textTransform: 'uppercase', letterSpacing: '-0.03em', marginBottom: '24px' } }, keyword ? React.createElement(React.Fragment, null,
                    "Resultados para ",
                    React.createElement("span", { style: { color: '#85C639' } },
                        "\"",
                        keyword,
                        "\"")) : 'Buscador'),
                React.createElement("form", { onSubmit: handleSearch, style: { display: 'flex', gap: '10px', maxWidth: '600px' } },
                    React.createElement("input", { type: "text", value: searchTerm, onChange: e => setSearchTerm(e.target.value), placeholder: "Buscar productos, usos, aplicaciones...", style: {
                            flex: 1, padding: '12px 16px', borderRadius: '8px', border: 'none', outline: 'none',
                            fontSize: '0.9rem', fontFamily: "'Inter', sans-serif", fontWeight: 500, color: '#1e293b',
                        } }),
                    React.createElement("button", { type: "submit", style: {
                            background: '#85C639', color: '#181B1C', border: 'none', borderRadius: '8px',
                            padding: '12px 24px', fontSize: '0.75rem', fontWeight: 800, letterSpacing: '0.08em',
                            textTransform: 'uppercase', cursor: 'pointer', fontFamily: "'Sora', sans-serif", whiteSpace: 'nowrap',
                        } }, "Buscar")))),
        React.createElement("section", { style: { maxWidth: '1536px', margin: '0 auto', padding: '48px 2rem' } },
            ...
            !result.fetching && items.length > 0 && (React.createElement(React.Fragment, null,
                ...
                React.createElement("div", { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: '20px' } }, familyGroups.map(({ family, products, representative }) => {
                    var _a;
                    return (React.createElement("a", { key: family, href: representative.url, style: { textDecoration: 'none', display: 'block', borderRadius: '12px', overflow: 'hidden', border: '1px solid #e2e8f0', transition: 'all 0.2s', background: '#fff' }, onMouseEnter: e => { const el = e.currentTarget; el.style.boxShadow = '0 8px 24px rgba(42,72,153,0.12)'; el.style.transform = 'translateY(-2px)'; }, onMouseLeave: e => { const el = e.currentTarget; el.style.boxShadow = 'none'; el.style.transform = 'none'; } },
                        React.createElement("div", { style: { aspectRatio: '1', background: '#f8faff', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center' } }, ((_a = representative.image) === null || _a === void 0 ? void 0 : _a.url) ? (React.createElement("img", { src: representative.image.url, alt: representative.image.alt || family, style: { width: '100%', height: '100%', objectFit: 'cover' } })) : (React.createElement("svg", { width: "40", height: "40", fill: "none", stroke: "#cbd5e1", viewBox: "0 0 24 24" },
                        ...
                        React.createElement("div", { style: { padding: '14px' } },
                            ...
                            React.createElement("div", { style: { display: 'flex', gap: '4px', flexWrap: 'wrap', marginTop: '8px' } },
                                products.slice(0, 4).map((p) => {
                                    ...
                                    return size ? (React.createElement("span", { key: p.productId, style: { fontSize: '0.62rem', fontWeight: 700, background: '#f1f5f9', color: '#475569', borderRadius: '4px', padding: '2px 6px', fontFamily: "'Inter', sans-serif" } }, size)) : null;
                                }),
                                products.length > 4 && (React.createElement("span", { style: { fontSize: '0.62rem', fontWeight: 700, background: '#e0e7ff', color: '#2A4899', borderRadius: '4px', padding: '2px 6px', fontFamily: "'Inter', sans-serif" } },
                                    "+",
                                    products.length - 4))))));
                })))))));
}
