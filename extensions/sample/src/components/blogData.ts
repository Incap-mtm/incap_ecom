...
// ── Contenido Markdown de los posts iniciales ────────────────────────────────
...
export function renderMarkdown(md: string): string {
  ...
  /**
   * Aplica inline Markdown a texto crudo.
   * Extrae links antes de escapar para poder validar el URL,
   * luego escapa el resto y aplica bold/italic/code.
   */
  /** Solo permite URLs de imagen http(s) o relativas a la raíz. */
  function safeImgUrl(url: string): string {
    const t = (url || '').trim();
    return /^(https?:|\/)/i.test(t) ? t : '';
  }

  function safeInline(raw: string): string {
    // 0. Extraer imágenes inline (![alt](url)) ANTES que los links, para que el
    //    `!` no rompa el patrón de link. Se guardan en el mismo buffer de tokens.
    const links: string[] = [];
    let s = raw.replace(/!\[([^\]]*)\]\(([^)]*)\)/g, (_m, alt, url) => {
      const safe = safeImgUrl(url);
      if (!safe) return '';
      const idx = links.length;
      links.push(
        `<img src="${escHtml(safe)}" alt="${escHtml(alt)}" loading="lazy" />`,
      );
      return `\x02LINK${idx}\x03`;
    });

    // 1. Extraer links (antes de escaping) para sanitizar URL
    s = s.replace(/\[([^\]]+)\]\(([^)]*)\)/g, (_m, text, url) => {
      const trimmed = url.trim();
      const safe = /^(https?:|\/|#|mailto:)/i.test(trimmed) ? trimmed : '#';
      const idx = links.length;
      links.push(
        `<a href="${escHtml(safe)}" target="_blank" rel="noopener noreferrer">${escHtml(text)}</a>`,
      );
      return `\x02LINK${idx}\x03`;
    });

    // 2. Escapar HTML en todo lo que queda
    s = escHtml(s);

    // 3. Inline Markdown (*, ** y ` no son caracteres HTML → sobreviven el escape)
    s = s
    ...
    // 4. Restaurar links
    s = s.replace(/\x02LINK(\d+)\x03/g, (_m, i) => links[parseInt(i, 10)]);
  ...
  while (i < lines.length) {
    ...
    // Imagen en su propia línea → figure con caption opcional (el alt).
    const im = line.match(/^!\[([^\]]*)\]\(([^)]*)\)\s*$/);
    if (im) {
      const alt = im[1].trim();
      const safe = safeImgUrl(im[2]);
      if (safe) {
        out.push(
          `<figure class="blog-fig"><img src="${escHtml(safe)}" alt="${escHtml(alt)}" loading="lazy" />${
            alt ? `<figcaption>${escHtml(alt)}</figcaption>` : ''
          }</figure>`,
        );
      }
      i++;
    ...
    if (hm) {
      ...
      out.push(`<h${level}>${safeInline(hm[2].trim())}</h${level}>`);
      i++;
      continue;
    }

    // Blockquote (> o >  — recolecta líneas consecutivas)
    if (line.startsWith('> ') || line === '>') {
      const bqParts: string[] = [];
      while (i < lines.length && (lines[i].startsWith('> ') || lines[i] === '>')) {
        bqParts.push(safeInline(lines[i].replace(/^>\s?/, '')));
        i++;
      }
      out.push(`<blockquote><p>${bqParts.join('</p><p>')}</p></blockquote>`);
      continue;
    }
    ...
    if (/^[-*]\s/.test(line)) {
      ...
      while (i < lines.length && /^[-*]\s/.test(lines[i])) {
        items.push(`<li>${safeInline(lines[i].replace(/^[-*]\s+/, ''))}</li>`);
        i++;
      }
    ...
    if (/^\d+\.\s/.test(line)) {
      ...
      while (i < lines.length && /^\d+\.\s/.test(lines[i])) {
        items.push(`<li>${safeInline(lines[i].replace(/^\d+\.\s+/, ''))}</li>`);
        i++;
      }
    ...
    // Párrafo — recolecta líneas hasta la siguiente línea vacía o bloque
    ...
    while (
      ...
      paraLines.push(safeInline(lines[i]));
      i++;
    }
...
export function parseBlogIndex(raw: string | null | undefined): BlogData {
  if (!raw) return DEFAULT_BLOG;
  try {
    const parsed = JSON.parse(raw);
    if (parsed && Array.isArray(parsed.posts) && parsed.posts.length > 0) {
      return parsed as BlogData;
    }
  } catch {
    // fallback
  }
