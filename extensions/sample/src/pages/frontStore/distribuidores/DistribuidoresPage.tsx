...
function lngToX(lng: number) { return ((lng - LNG_MIN) / (LNG_MAX - LNG_MIN)) * MAP_W; }
...
function MapPin({ cx, cy, active, onClick }: { cx: number; cy: number; active: boolean; onClick: () => void }) {
  const fill = active ? '#85C639' : '#2A4899';
  const stroke = '#fff';
  return (
    <g transform={`translate(${cx},${cy})`} onClick={onClick} style={{ cursor: 'pointer' }}>
      {/* Drop shadow */}
      <ellipse cx={0} cy={3} rx={6} ry={2.5} fill="rgba(0,0,0,0.2)" />
      {/* Pin tail */}
      <path d="M -6,-10 Q 0,4 0,0 Q 0,4 6,-10 Z" fill={fill} />
      {/* Pin head circle */}
      <circle cx={0} cy={-21} r={13} fill={fill} stroke={stroke} strokeWidth={2} />
      {/* Location dot inside */}
      <circle cx={0} cy={-21} r={4} fill={stroke} opacity={0.9} />
      {/* Active pulse ring */}
      {active && (
        <circle cx={0} cy={-21} r={17} fill="none" stroke="#85C639" strokeWidth={1.5} strokeDasharray="4 2" opacity={0.8} />
      )}
    </g>
  );
}
...
export default function DistribuidoresPage() {
  ...
  return (
    <div style={{ fontFamily: 'Sora, Inter, sans-serif', background: '#f8fafc' }}>
      ...
      {/* Header */}
      <div style={{ background: 'linear-gradient(135deg, #2A4899 0%, #1e3576 100%)', padding: '3rem 2rem 2.5rem', textAlign: 'center' }}>
        ...
        <p style={{ color: 'rgba(255,255,255,0.6)', fontSize: '14px', marginTop: '10px' }}>
          {loaded ? `${data.length} distribuidores en todo el país` : 'Cargando...'}
        </p>
      </div>

      {/* Body */}
      <div style={{ display: 'flex', minHeight: '680px' }}>

        {/* Sidebar */}
        <div style={{ width: '380px', minWidth: '300px', display: 'flex', flexDirection: 'column', background: '#fff', borderRight: '1px solid #e2e8f0' }}>

          {/* Filters */}
          <div style={{ padding: '16px', borderBottom: '1px solid #e2e8f0', flexShrink: 0 }}>
            <div style={{ marginBottom: '10px' }}>
              <span style={labelStyle}>Buscar distribuidor</span>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
              ...
              <div>
                ...
                <select value={industria} onChange={e => setIndustria(e.target.value)} style={{ ...inputStyle, cursor: 'pointer' }}>
                  <option value="">Todas</option>
                  {uniqueIndustrias.map(i => <option key={i} value={i}>{LABEL_INDUSTRIA[i] || i}</option>)}
                </select>
              </div>
            </div>
            <div style={{ marginTop: '10px', fontSize: '11px', color: '#94a3b8', display: 'flex', alignItems: 'center', gap: '8px' }}>
              <span>{filtered.length} distribuidor{filtered.length !== 1 ? 'es' : ''}</span>
              {(search || region || industria) && (
                <button onClick={() => { setSearch(''); setRegion(''); setIndustria(''); setActiveCity(null); }}
                  style={{ color: '#2A4899', background: 'none', border: 'none', cursor: 'pointer', fontSize: '11px', fontWeight: 700, padding: 0 }}>
                  Limpiar
                </button>
              )}
            </div>
          ...
          {/* City list */}
          <div style={{ overflowY: 'auto', flex: 1, maxHeight: '580px' }}>
            {Object.entries(byCity).sort((a, b) => b[1].length - a[1].length).map(([city, dists]) => (
              <button key={city}
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  ...
                  <svg width="10" height="14" viewBox="0 0 10 14">
                    <circle cx="5" cy="5" r="5" fill={activeCity === city ? '#2A4899' : '#cbd5e1'} />
                    <polygon points="3,8 7,8 5,13" fill={activeCity === city ? '#2A4899' : '#cbd5e1'} />
                  </svg>
                  <span style={{ fontSize: '12px', fontWeight: 800, color: activeCity === city ? '#2A4899' : '#374151', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
                    {city}
                  </span>
                </div>
                <span style={{ fontSize: '11px', background: activeCity === city ? '#2A4899' : '#f1f5f9', color: activeCity === city ? '#fff' : '#64748b', borderRadius: '20px', padding: '2px 8px', fontWeight: 700, transition: 'all 0.2s', minWidth: '22px', textAlign: 'center' }}>
                  {dists.length}
                </span>
              </button>
            ))}
        ...
        {/* Map area */}
        <div style={{ flex: 1, background: 'linear-gradient(160deg, #dbeafe 0%, #e8f0fe 100%)', position: 'relative', overflow: 'hidden', minHeight: '680px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>

          <svg
            viewBox={`0 0 ${MAP_W} ${MAP_H}`}
            style={{ width: '100%', height: '100%', maxWidth: '520px', maxHeight: '660px' }}
            onClick={(e) => {
              const tag = (e.target as SVGElement).tagName;
              if (tag === 'svg' || tag === 'path') setActiveCity(null);
            }}
          >
            <g style={{ transform: zoomTransform, transformOrigin: '0 0', transition: 'transform 0.5s cubic-bezier(0.4,0,0.2,1)' }}>

              {/* Colombia fill */}
              <path d={COLOMBIA_PATH} fill="#bfdbfe" stroke="#93c5fd" strokeWidth="1" strokeLinejoin="round" />

              {/* Dim overlay when city selected */}
              {activeCity && <path d={COLOMBIA_PATH} fill="rgba(30,53,118,0.2)" stroke="none" />}
              ...
              {Object.entries(byCity).map(([city, dists]) => {
                ...
                return (
                  <g key={city}>
                    {/* City label — always visible, scaled down when zoomed */}
                    {!activeCity && (
                      <text
                        x={cx} y={cy - 38}
                        textAnchor="middle"
                    ...
                    <MapPin cx={cx} cy={cy} active={isActive} onClick={() => setActiveCity(isActive ? null : city)} />
                  </g>
                );
              })}
            </g>
          ...
          {activeCity && activeCityData && (
            <div style={{
              ...
              {/* Popup header */}
              <div style={{ background: 'linear-gradient(135deg, #2A4899, #1e3576)', padding: '16px', flexShrink: 0 }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '8px' }}>
                  <div>
                    <div style={{ fontSize: '10px', color: '#85C639', fontWeight: 700, letterSpacing: '0.2em', textTransform: 'uppercase', marginBottom: '4px' }}>
                      {LABEL_REGION[activeCityData[0]?.region] || activeCityData[0]?.region}
                    </div>
                    <div style={{ fontSize: '16px', fontWeight: 900, color: '#fff', textTransform: 'uppercase', letterSpacing: '0.04em' }}>
                      {activeCity}
                    </div>
                  ...
                  <button onClick={() => setActiveCity(null)}
                    style={{ background: 'rgba(255,255,255,0.15)', border: 'none', borderRadius: '50%', width: '28px', height: '28px', cursor: 'pointer', color: '#fff', fontSize: '18px', lineHeight: '28px', textAlign: 'center', flexShrink: 0 }}>
                    ×
                  </button>
                </div>
              </div>

              {/* Distributor list with sequential numbers */}
                ...
                {activeCityData.map((d, i) => (
                  <div key={i}
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ display: 'inline-block', fontSize: '10px', color: '#2A4899', background: '#eff6ff', borderRadius: '4px', padding: '2px 6px', fontWeight: 700, marginBottom: '5px' }}>
                        {LABEL_INDUSTRIA[d.industria] || d.industria}
                      </div>
          ...
          {/* Hint */}
          {!activeCity && loaded && (
            <div style={{ position: 'absolute', bottom: '16px', left: '50%', transform: 'translateX(-50%)', background: 'rgba(255,255,255,0.92)', backdropFilter: 'blur(8px)', borderRadius: '10px', padding: '8px 16px', fontSize: '11px', color: '#64748b', fontWeight: 600, whiteSpace: 'nowrap', boxShadow: '0 2px 12px rgba(0,0,0,0.08)' }}>
              Seleccioná un pin o una ciudad en la lista
            </div>
          )}
