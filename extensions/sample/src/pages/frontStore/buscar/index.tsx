      ...
      <section style={{ background: 'linear-gradient(160deg, #2A4899 0%, #1e3576 100%)', padding: '40px 0 48px' }}>
        <div style={{ maxWidth: '1536px', margin: '0 auto', padding: '0 2rem' }}>
          ...
          <h1 style={{ fontSize: 'clamp(2rem, 5vw, 3.5rem)', fontWeight: 900, color: '#fff', fontFamily: "'Sora', sans-serif", lineHeight: 1.1, textTransform: 'uppercase', letterSpacing: '-0.03em', marginBottom: '24px' }}>
            {keyword ? <>Resultados para <span style={{ color: '#85C639' }}>"{keyword}"</span></> : 'Buscador'}
          </h1>

          {/* Search input en la página */}
          <form onSubmit={handleSearch} style={{ display: 'flex', gap: '10px', maxWidth: '600px' }}>
            <input
              type="text"
              value={searchTerm}
              onChange={e => setSearchTerm(e.target.value)}
              placeholder="Buscar productos, usos, aplicaciones..."
              style={{
        ...
        {!result.fetching && items.length > 0 && (
          <>
            <p style={{ fontSize: '0.8rem', color: '#64748b', fontFamily: "'Inter', sans-serif", marginBottom: '32px', fontWeight: 500 }}>
              <strong style={{ color: '#2A4899' }}>{total}</strong> {total === 1 ? 'producto encontrado' : 'productos encontrados'}
              {keyword && <> para <strong>"{keyword}"</strong></>}
            </p>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: '20px' }}>
              {familyGroups.map(({ family, products, representative }) => (
                <a
                  ...
                  style={{ textDecoration: 'none', display: 'block', borderRadius: '12px', overflow: 'hidden', border: '1px solid #e2e8f0', transition: 'all 0.2s', background: '#fff' }}
                  ...
                  <div style={{ aspectRatio: '1', background: '#f8faff', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    {representative.image?.url ? (
                      ...
                      <svg width="40" height="40" fill="none" stroke="#cbd5e1" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 20M6 8h.01M4 4h16a2 2 0 012 2v12a2 2 0 01-2 2H4a2 2 0 01-2-2V6a2 2 0 012-2z" />
                      </svg>
                    )}
                  </div>
                  <div style={{ padding: '14px' }}>
                    ...
                    <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap', marginTop: '8px' }}>
                      {products.slice(0, 4).map((p: any) => {
                        ...
                        return size ? (
                          <span key={p.productId} style={{ fontSize: '0.62rem', fontWeight: 700, background: '#f1f5f9', color: '#475569', borderRadius: '4px', padding: '2px 6px', fontFamily: "'Inter', sans-serif" }}>
                            {size}
                          </span>
                        ) : null;
                      })}
                      {products.length > 4 && (
                        <span style={{ fontSize: '0.62rem', fontWeight: 700, background: '#e0e7ff', color: '#2A4899', borderRadius: '4px', padding: '2px 6px', fontFamily: "'Inter', sans-serif" }}>
                          +{products.length - 4}
                        </span>
                      )}
                    </div>
