...
function PostCard({ post }: { post: BlogPost }) {
  return (
    <a
      ...
      onMouseEnter={(e) => {
        (e.currentTarget as HTMLAnchorElement).style.boxShadow = '0 8px 30px rgba(42,72,153,0.15)';
        (e.currentTarget as HTMLAnchorElement).style.transform = 'translateY(-2px)';
      }}
      onMouseLeave={(e) => {
        (e.currentTarget as HTMLAnchorElement).style.boxShadow = '0 2px 12px rgba(42,72,153,0.06)';
    ...
    >
      ...
      <div style={{ padding: '1.25rem 1.5rem 1.5rem', flex: 1, display: 'flex', flexDirection: 'column' }}>
        ...
        {/* Date */}
        <span style={{ fontSize: '11px', color: '#94a3b8', fontFamily: 'Sora, sans-serif', marginBottom: '8px' }}>
...
function FeaturedPost({ post }: { post: BlogPost }) {
  return (
    ...
    >
      {/* Cover */}
      <div style={{ position: 'relative', minHeight: '260px', background: '#f1f5f9' }}>
        <img
          src={post.cover}
          alt={post.title}
      ...
      <div style={{ padding: '2rem 2.5rem', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
        ...
        <span style={{ fontSize: '12px', color: '#94a3b8', fontFamily: 'Sora, sans-serif', marginBottom: '10px' }}>
          {formatDate(post.date)}
        </span>
        <h2
          style={{
        ...
        <p style={{ fontSize: '14px', color: '#64748b', lineHeight: 1.7, margin: '0 0 1.5rem', fontFamily: 'Sora, sans-serif' }}>
          {post.excerpt}
        </p>
        <span
          style={{
...
export default function BlogPage() {
  ...
  const blogData = parseBlogIndex(result.data?.setting?.blogIndex);

  // Ordenar por fecha descendente
  const sortedPosts = useMemo(
    () => [...blogData.posts].sort((a, b) => b.date.localeCompare(a.date)),
    [blogData.posts],
  );
    ...
    <div style={{ fontFamily: 'Sora, sans-serif', background: '#f8fafc', minHeight: '100vh' }}>
      ...
      >
        ...
        <div style={{ maxWidth: '1200px', margin: '0 auto', position: 'relative' }}>
          <span style={S.kicker}>Blog INCAP</span>
          <h1 style={S.h1}>
            Conocimiento técnico<br />
            <span style={{ color: VERDE }}>para la industria</span>
          </h1>
          <p
            style={{
              fontSize: '16px',
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '3rem 2rem 5rem' }}>
        ...
        {/* Tag filter */}
        {allTags.length > 0 && (
          <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginBottom: '2rem', alignItems: 'center' }}>
            <span style={{ fontSize: '9px', fontWeight: 700, color: '#94a3b8', letterSpacing: '0.2em', textTransform: 'uppercase', marginRight: '4px' }}>
              Filtrar
            </span>
            <TagChip label="Todos" active={!activeTag} onClick={() => handleTagChange('')} />
            {allTags.map((tag) => (
              <TagChip key={tag} label={tag} active={activeTag === tag} onClick={() => handleTagChange(tag)} />
            ))}
          </div>
        )}

        {/* Featured post (solo cuando no hay filtro activo y hay featured) */}
        ...
        {pagedPosts.length > 0 ? (
          ...
          >
            <p style={{ color: '#94a3b8', fontSize: '15px', fontFamily: 'Sora, sans-serif', margin: 0 }}>
