  ...
  try {
    ...
    for (const root of ROOTS) {
      ...
      for (const f of files) {
        const webp = toWebp(f);
        if (fs.existsSync(webp)) { yaOptimizadas++; continue; }
        if (convertidas >= MAX_PER_RUN) { pendientes++; continue; }
        try { await sharp(f).webp({ quality: 82 }).toFile(webp); convertidas++; }
        catch { errores++; }
      }
    }
    ...
    return response.json({
      ...
      dbSinWebp,
      message:
        (pendientes > 0 ? `Quedaron ${pendientes} por convertir — re-ejecutá. ` : '') +
