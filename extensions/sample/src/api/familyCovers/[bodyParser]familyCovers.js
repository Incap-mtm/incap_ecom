  ...
  if (request.method === 'GET') {
    try {
      const { rows } = await pool.query(
        `SELECT p.uuid, pd.name, pi.origin_image AS image
         FROM product p
      ...
      const { rows: settingRows } = await pool.query(
        `SELECT value FROM setting WHERE name = 'family_covers' LIMIT 1`
      );
      let covers = {};
      try {
        covers = JSON.parse(settingRows[0]?.value || '{}');
      } catch {
        covers = {};
      }
      if (!covers || typeof covers !== 'object' || Array.isArray(covers)) covers = {};
