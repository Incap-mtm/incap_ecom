...
export default async function syncVariants(request, response) {
  ...
  try {
    ...
    for (const p of products) {
      ...
      if (normSize !== rawSize) {
        ...
        await client.query(
          ...
          [newName, p.product_description_id]
        );
        p.name = newName;
        renamed++;
      }
    ...
    for (const [, ps] of toMigrate) ps.forEach((p) => p.size && sizesNeeded.add(p.size));
    ...
    for (const [, ps] of toMigrate) {
      ...
      if (existingGroupId !== null && newProducts.length === 0) { skipped++; continue; }
