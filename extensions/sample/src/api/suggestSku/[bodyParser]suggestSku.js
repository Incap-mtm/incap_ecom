...
import { encodeSizeToken } from '../../lib/encodeSize.js';
...
export default async function suggestSku(request, response) {
  try {
    ...
    if (siblingsResult.rows.length > 0) {
      ...
      const prefixCheckResult = await pool.query(
        `SELECT DISTINCT UPPER(LEFT(sku, 2)) AS seg1
         FROM product
         WHERE sku <> '' AND sku IS NOT NULL AND UPPER(LEFT(sku, 2)) = $1`,
        [seg1]
      );
