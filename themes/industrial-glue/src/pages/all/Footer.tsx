...
export default function Footer({ setting }: FooterProps) {
  ...
  const facebook  = setting?.storeFacebook  ?? 'https://www.facebook.com/share/1D7BbLWtgs/?mibextid=wwXIfr';
  const linkedin  = setting?.storeLinkedin  ?? 'https://www.linkedin.com/company/incap-s-a/?viewAsMember=true';
  const tiktok    = setting?.storeTiktok    ?? 'https://www.tiktok.com/@incap.s.a';
  const youtube   = setting?.storeYoutube   ?? 'https://www.youtube.com/@IncapSA_Col';
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '3rem', marginBottom: '3rem' }}>
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
              ...
              ].map(({ href, title, fill, path }) => (
                <a key={title} href={href} target="_blank" rel="noopener noreferrer" title={title}
                  style={{ width: '34px', height: '34px', borderRadius: '8px', background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'background 0.2s', flexShrink: 0 }}>
                  <svg width="16" height="16" viewBox="0 0 24 24" fill={fill}><path d={path} /></svg>
                </a>
              ))}
            </div>
          </div>
