...
export default function LogoStrip() {
    ...
    return (React.createElement("section", { className: "bg-white border-y border-slate-100 py-10 overflow-hidden" },
        React.createElement("div", { className: "relative flex overflow-hidden" },
            ...
            React.createElement("div", { className: "flex animate-marquee whitespace-nowrap items-center py-4 grayscale hover:grayscale-0 transition-all duration-1000 opacity-40 hover:opacity-100 gap-[50px]" }, loopLogos.map((logo, i) => (React.createElement("img", { key: `${logo.name}-${i}`, src: logo.src, alt: logo.name, className: "h-10 w-auto object-contain flex-shrink-0 transition-transform hover:scale-110", style: { mixBlendMode: 'multiply' } })))))));
}
export const layout = {
    areaId: 'content',
    sortOrder: 3
