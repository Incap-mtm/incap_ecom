...
export default function TechnicalSheet({ product }) {
    ...
    return (React.createElement("div", { className: "mt-10 border-t border-slate-100 pt-10" },
        ...
        React.createElement("button", { onClick: () => setStatus('open'), className: "inline-flex items-center gap-3 px-6 py-3 bg-[#2A4899] hover:bg-[#1e3a7a] text-white font-semibold rounded-lg transition-all duration-300 group", style: { fontFamily: 'Sora, sans-serif' } },
        ...
        (status === 'open' || status === 'submitting' || status === 'success' || status === 'error') && (React.createElement("div", { style: { position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.55)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }, onClick: (e) => { if (e.target === e.currentTarget)
            ...
            React.createElement("div", { style: { background: '#fff', borderRadius: '20px', padding: '2rem', width: '100%', maxWidth: '420px', boxShadow: '0 24px 64px rgba(0,0,0,0.2)', fontFamily: 'Sora, sans-serif' } }, status === 'success' ? (React.createElement("div", { style: { textAlign: 'center', padding: '1rem 0' } },
                ...
                React.createElement("p", { style: { fontSize: '13px', color: '#64748b', lineHeight: 1.6, marginBottom: '1.5rem' } },
                    ...
                    "."),
                React.createElement("button", { onClick: close, style: { padding: '10px 24px', background: '#2A4899', color: '#fff', border: 'none', borderRadius: '10px', fontWeight: 700, cursor: 'pointer', fontSize: '13px' } }, "Cerrar"))) : (React.createElement(React.Fragment, null,
                React.createElement("div", { style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.5rem' } },
                    ...
                    React.createElement("button", { onClick: close, style: { background: 'none', border: 'none', cursor: 'pointer', color: '#94a3b8', padding: '4px' } },
                ...
                React.createElement("form", { onSubmit: handleSubmit, style: { display: 'flex', flexDirection: 'column', gap: '14px' } },
                    ['nombre', 'email', 'telefono'].map((field) => (React.createElement("div", { key: field },
                        ...
                        React.createElement("input", { type: field === 'email' ? 'email' : field === 'telefono' ? 'tel' : 'text', required: true, value: form[field], onChange: (e) => setForm(f => ({ ...f, [field]: e.target.value })), disabled: status === 'submitting', placeholder: field === 'nombre' ? 'Tu nombre' : field === 'email' ? 'correo@empresa.com' : '+57 300 000 0000', style: { width: '100%', padding: '10px 14px', border: '1.5px solid #e2e8f0', borderRadius: '10px', fontSize: '13px', fontFamily: 'Inter, sans-serif', outline: 'none', boxSizing: 'border-box', background: status === 'submitting' ? '#f8fafc' : '#fff', color: '#181B1C' }, onFocus: (e) => { e.target.style.borderColor = '#2A4899'; }, onBlur: (e) => { e.target.style.borderColor = '#e2e8f0'; } })))),
                    ...
                    React.createElement("button", { type: "submit", disabled: status === 'submitting', style: { padding: '12px', background: status === 'submitting' ? '#94a3b8' : '#2A4899', color: '#fff', border: 'none', borderRadius: '12px', fontWeight: 800, cursor: status === 'submitting' ? 'default' : 'pointer', fontSize: '13px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', transition: 'background 0.2s', marginTop: '4px' } }, status === 'submitting' ? (React.createElement(React.Fragment, null,
                        React.createElement("svg", { style: { animation: 'spin 1s linear infinite' }, width: "16", height: "16", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24" },
                            React.createElement("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" })),
                        "Enviando\u2026")) : (React.createElement(React.Fragment, null,
